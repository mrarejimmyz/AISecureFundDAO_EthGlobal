...
const completedProposals = [
  {
    ...
    description: 'Implementation of zero-knowledge proofs for DAO governance to enhance privacy.',
    votesFor: 284,
    votesAgainst: 34,
    votesAbstain: 12,
    totalVotes: 330,
  ...
  },
  {
    ...
    description: 'Extend trusted execution environment capabilities to support multiple blockchain networks.',
    votesFor: 190,
    votesAgainst: 105,
    votesAbstain: 25,
    totalVotes: 320,
  ...
  },
  {
    ...
    description: 'Research and develop standards for NFTs with privacy-preserving attributes and ownership.',
    votesFor: 150,
    votesAgainst: 170,
    votesAbstain: 5,
    totalVotes: 325,
  ...
  },
  {
    ...
    description: 'Fund educational content on privacy-preserving technologies and TEEs for wider adoption.',
    votesFor: 315,
    votesAgainst: 5,
    votesAbstain: 10,
    totalVotes: 330,
...
const completedAuctions = [
  {
    ...
    description: 'Funding for research into AI-driven privacy-preserving analytics.',
    winningBid: 280,
    totalBids: 14,
    winner: '0x7F2e...A1dB',
  ...
  },
  {
    ...
    description: 'Development of zero-knowledge proof infrastructure for privacy-preserving applications.',
    winningBid: 350,
    totalBids: 8,
    winner: '0x3A5b...C7d9',
  ...
  },
  {
    ...
    description: 'Development of an interactive dashboard for DAO governance tracking and visualization.',
    winningBid: 120,
    totalBids: 6,
    winner: '0xB2c8...9F4e',
...
const ResultsVisualization: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'proposals' | 'auctions' | 'analytics'>('proposals');
  const [selectedProposal, setSelectedProposal] = useState<string | null>(null);
  const [selectedAuction, setSelectedAuction] = useState<string | null>(null);

  // Calculate total funding
  const totalFunding = fundingDistributionData.reduce((sum, item) => sum + item.amount, 0);

  const renderStatusBadge = (status: string) => {
    const isVerified = status === 'verified';
    return (
      <div className={`${styles.statusBadge} ${isVerified ? styles.verifiedBadge : styles.pendingBadge}`}>
        {isVerified ? (
          <>
  ...
  const renderProposalVotingResults = (proposal: typeof completedProposals[0]) => {
      ...
      <div className={styles.votingResults}>
        ...
        <div className={styles.votingLegend}>
          <div className={styles.legendItem}>
            ...
            <div className={styles.legendLabel}>For: {proposal.votesFor} votes</div>
          </div>
          <div className={styles.legendItem}>
            <div className={styles.legendColor} style={{ backgroundColor: '#ef4444' }}></div>
            <div className={styles.legendLabel}>Against: {proposal.votesAgainst} votes</div>
          </div>
          <div className={styles.legendItem}>
            <div className={styles.legendColor} style={{ backgroundColor: '#94a3b8' }}></div>
            <div className={styles.legendLabel}>Abstain: {proposal.votesAbstain} votes</div>
          </div>
        </div>
      </div>
    );
      ...
      <div className={styles.resultsSection}>
        <div className={styles.resultsList}>
          {completedProposals.map(proposal => (
            <div 
              key={proposal.id}
              className={`${styles.resultCard} ${selectedProposal === proposal.id ? styles.selectedCard : ''}`}
              onClick={() => setSelectedProposal(proposal.id)}
            >
              <div className={styles.resultHeader}>
                <span className={styles.resultId}>{proposal.id}</span>
                <div className={`${styles.resultStatus} ${proposal.status === 'passed' ? styles.passedStatus : styles.rejectedStatus}`}>
        ...
        {selectedProposal && (
          <div className={styles.resultDetails}>
            ...
            {renderAiInsights(
              completedProposals.find(p => p.id === selectedProposal)?.aiInsights || []
            )}
              ...
              <div className={styles.verificationSteps}>
                <div className={styles.verificationStep}>
                  ...
                  <div className={styles.verificationContent}>
                    <span className={styles.verificationTitle}>Encrypted Votes</span>
                ...
                <div className={styles.verificationStep}>
                  ...
                  <div className={styles.verificationContent}>
                    <span className={styles.verificationTitle}>TEE Computation</span>
                    <span className={styles.verificationStatus}>Verified</span>
                  </div>
                </div>
                ...
                <div className={styles.verificationStep}>
                  ...
                  <div className={styles.verificationContent}>
                    <span className={styles.verificationTitle}>Results Published</span>
                    <span className={styles.verificationStatus}>Verified</span>
                  </div>
                </div>
              </div>
      ...
      <div className={styles.resultsSection}>
        <div className={styles.resultsList}>
          {completedAuctions.map(auction => (
            <div 
              key={auction.id}
              className={`${styles.resultCard} ${selectedAuction === auction.id ? styles.selectedCard : ''}`}
            ...
            >
              ...
              <div className={styles.auctionResultDetails}>
                <div className={styles.auctionResultDetail}>
                  ...
                  <span className={styles.detailValue}>{auction.winningBid} ETH</span>
                </div>
                
                <div className={styles.auctionResultDetail}>
                  <span className={styles.detailLabel}>Total Bids</span>
                  <span className={styles.detailValue}>{auction.totalBids}</span>
                </div>
        ...
        {selectedAuction && (
          <div className={styles.resultDetails}>
            ...
            {renderAiInsights(
              completedAuctions.find(a => a.id === selectedAuction)?.aiInsights || []
            )}
            ...
            <div className={styles.teeVerificationSection}>
              ...
              <div className={styles.verificationSteps}>
                <div className={styles.verificationStep}>
                  ...
                  <div className={styles.verificationContent}>
                    <span className={styles.verificationTitle}>Sealed Bids</span>
                    <span className={styles.verificationStatus}>Verified</span>
                  </div>
                </div>
      ...
      <div className={styles.analyticsSection}>
          ...
          <div className={styles.pieChartContainer}>
            <div className={styles.pieChart}>
              {fundingDistributionData.map((segment, index) => {
                ...
                const total = fundingDistributionData.reduce((sum, data) => sum + data.amount, 0);
                const angle = (segment.amount / total) * 360;
                const previousSegmentsAngle = fundingDistributionData
                  .slice(0, index)
                  .reduce((sum, data) => sum + data.amount, 0) / total * 360;
            ...
            <div className={styles.pieChartLegend}>
              {fundingDistributionData.map(segment => (
                <div key={segment.category} className={styles.legendItem}>
                  <div className={styles.legendColor} style={{ backgroundColor: segment.color }}></div>
                  <div className={styles.legendContent}>
                    <span className={styles.legendLabel}>{segment.category}</span>
                    <span className={styles.legendValue}>{segment.amount} ETH</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className={styles.analyticsCard}>
          ...
          <div className={styles.barChartContainer}>
            {quarterlyFundingData.map((quarter) => {
              const maxAmount = Math.max(...quarterlyFundingData.map(d => d.amount));
              ...
              return (
                <div key={quarter.quarter} className={styles.barChartColumn}>
                  <div className={styles.barChartBar}>
                    ...
                    >
                      <span className={styles.barChartValue}>{quarter.amount}</span>
                    </div>
                  </div>
        ...
        <div className={styles.aiSummaryCard}>
          <div className={styles.aiSummaryHeader}>
            ...
            <h3 className={styles.aiSummaryTitle}>AI-Enhanced Funding Analysis</h3>
          </div>
          
          <div className={styles.aiSummaryContent}>
            <p className={styles.aiSummaryText}>
              Based on historical data and current trends, our AI analysis indicates strong growth in funding 
              allocations, with a 44% increase from Q1 2024 to Q1 2025. Development and research categories 
              continue to receive the largest share of funding (56%), aligned with the DAO's technical objectives.
            </p>
            ...
            <div className={styles.aiInsights}>
              <div className={styles.aiInsightItem}>
                ...
                <div className={styles.aiInsightContent}>
                  ...
                  <p className={styles.aiInsightText}>
                    Projected 30-35% increase in total funding for Q2 2025 based on current proposal pipeline 
              ...
              <div className={styles.aiInsightItem}>
                ...
                <div className={styles.aiInsightContent}>
                  ...
                  <p className={styles.aiInsightText}>
                    Infrastructure funding is growing fastest (+45% QoQ), reflecting increased focus on 
                    scalability and interoperability initiatives.
                  </p>
                </div>
              </div>
              ...
              <div className={styles.aiInsightItem}>
                ...
                <div className={styles.aiInsightContent}>
                  <h4 className={styles.aiInsightTitle}>Participation Analysis</h4>
                  <p className={styles.aiInsightText}>
                    Voting participation has increased by 25% since implementing TEE-protected privacy features, 
                    suggesting enhanced community trust in the governance process.
                  </p>
                </div>
              </div>
            </div>
    ...
    <div className={styles.resultsContainer}>
      <div className={styles.resultsHeader}>
        ...
        <p className={styles.resultsDescription}>
          View the outcomes of proposal votes and auctions, all verified through our TEE network.
          AI-enhanced analytics provide deeper insights into funding trends and governance patterns.
        </p>
      </div>
      
      <div className={styles.resultsTabs}>
        <button
          className={`${styles.resultsTab} ${activeTab === 'proposals' ? styles.activeTab : ''}`}
        ...
        <button
          className={`${styles.resultsTab} ${activeTab === 'auctions' ? styles.activeTab : ''}`}
        ...
        <button
          className={`${styles.resultsTab} ${activeTab === 'analytics' ? styles.activeTab : ''}`}
      ...
      <div className={styles.teeExplanation}>
        ...
        <div className={styles.teeContent}>
          ...
          <p className={styles.teeText}>
            All voting and auction results are processed and verified through our network of Trusted 
            Execution Environments (TEEs). This ensures complete integrity of the results while 
            preserving the privacy of votes and bids.
          </p>
        </div>
